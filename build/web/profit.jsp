            ...
            <div class="container">
                ...
                <div class="contact-form">
                    <table width="100%" align="center" border="2">
                        <tr align="center"><td>Crop ID</td><td>Crop Name</td><td>Soil Type</td><td>Yield of Crop</td><td>Cast(kg)</td></tr>
                        <%
                            try {
                                Class.forName("com.mysql.jdbc.Driver");
                                Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/crop", "root", "admin");
                                Statement st = con.createStatement();
                                st.execute("TRUNCATE TABLE profit");
                                ...
                                while (rs.next()) {
                                    ...
                                    ResultSet rs1 = st1.executeQuery("select * from yearwise where cropid='" + i + "'");
                                    while (rs1.next()) {
                                        crop += Integer.parseInt(rs1.getString("quantity"));
                                        avg = crop / 3;
                                        if (j == 3) {
                        ...
                        <tr align="center"><td><%=rs.getString("cropid")%></td><td><%=rs.getString("cropname")%></td><td><%=rs.getString("soiltype")%></td><td><%=avg%></td><td><%=rs.getString("cost")%></td></tr>
                        <%
                        pro=avg*rs.getInt("cost");
                        Statement st2 = con.createStatement();
                        st2.execute("insert into profit(cropid,yield,cprofit) values('"+rs.getString("cropid")+"','"+avg+"','"+pro+"')");
                    ...
                    <form action="final.jsp"><input type="submit" value="Next"/></form>
